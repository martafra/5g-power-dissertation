#!/bin/bash
# Matrix experiments for 2CU-4DU topology
...
test_mode:
  test_ue:
    ...
    pdsch_active: true
TMEOF
  cat > "$CONFIGS/testmode_b2.yml" << TMEOF
test_mode:
  test_ue:
    rnti: 0x288
    ri: 1
    ...
    pdsch_active: true
TMEOF
  cat > "$CONFIGS/testmode_b3.yml" << TMEOF
test_mode:
  test_ue:
    rnti: 0x344
    ri: 1
...
echo "=== srsRAN Power Matrix Experiment: 2CU-4DU ==="
...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    for RUN in $(seq 1 $RUNS); do
      ...
      COUNT=$((COUNT + 1))
      echo ""
      echo "[$COUNT/$TOTAL] 2CU-4DU | CQI=$CQI | UEs=$NUE | Run=$RUN"
      set_testmode $NUE $CQI
      docker stop srsran_du srsran_du2 srsran_du3 srsran_du4 srsran_du_b srsran_du_b2 srsran_du_b3 srsran_du_b4 2>/dev/null
      docker rm srsran_du srsran_du2 srsran_du3 srsran_du4 srsran_du_b srsran_du_b2 srsran_du_b3 srsran_du_b4 2>/dev/null
      $COMPOSE up -d du du2 du3 du4 du-b du-b2 du-b3 du-b4 2>/dev/null
      sleep 10
      measure "2cu8du" $NUE $CQI $RUN
...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    echo -n "2cu8du CQI=${CQI} ${NUE}UE: "
    python3 - << PYEOF
import json, glob, numpy as np
files = glob.glob("${LOGDIR}/power_2cu8du_cqi${CQI}_${NUE}ue_run*.json")
means = []
