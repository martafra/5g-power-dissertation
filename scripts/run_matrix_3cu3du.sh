...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    for RUN in $(seq 1 $RUNS); do
      ...
      docker stop srsran_du srsran_du_b srsran_du_c 2>/dev/null
      docker rm srsran_du srsran_du_b srsran_du_c 2>/dev/null
      $COMPOSE up -d du du-b du-c 2>/dev/null
      sleep 10
      measure "3cu3du" $NUE $CQI $RUN
...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    echo -n "3cu3du CQI=${CQI} ${NUE}UE: "
    python3 - << PYEOF
import json, glob, numpy as np
files = glob.glob("${LOGDIR}/power_3cu3du_cqi${CQI}_${NUE}ue_run*.json")
means = []
