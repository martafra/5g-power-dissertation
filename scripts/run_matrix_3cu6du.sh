...
test_mode:
  test_ue:
    ...
    pdsch_active: true
TMEOF
  cat > "$CONFIGS/testmode_c.yml" << TMEOF
test_mode:
  test_ue:
    rnti: 0x400
    ri: 1
    ...
    pdsch_active: true
TMEOF
  cat > "$CONFIGS/testmode_c2.yml" << TMEOF
test_mode:
  test_ue:
    rnti: 0x460
    ri: 1
    ...
    pdsch_active: true
TMEOF
  cat > "$CONFIGS/testmode_c3.yml" << TMEOF
test_mode:
  test_ue:
    rnti: 0x4C0
    ri: 1
...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    for RUN in $(seq 1 $RUNS); do
      ...
      docker stop srsran_du srsran_du2 srsran_du_b srsran_du_b2 srsran_du_c srsran_du_c2 srsran_du_c3 2>/dev/null
      docker rm srsran_du srsran_du2 srsran_du_b srsran_du_b2 srsran_du_c srsran_du_c2 srsran_du_c3 2>/dev/null
      $COMPOSE up -d du du2 du-b du-b2 du-c du-c2 du-c3 2>/dev/null
      sleep 10
