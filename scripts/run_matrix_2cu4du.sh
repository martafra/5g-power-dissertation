...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    for RUN in $(seq 1 $RUNS); do
      ...
      docker stop srsran_du srsran_du2 srsran_du_b srsran_du_b2 2>/dev/null
      docker rm srsran_du srsran_du2 srsran_du_b srsran_du_b2 2>/dev/null
      $COMPOSE up -d du du2 du-b du-b2 2>/dev/null
      sleep 10
      measure "2cu4du" $NUE $CQI $RUN
...
for CQI in "${CQI_VALUES[@]}"; do
  for NUE in "${UE_VALUES[@]}"; do
    echo -n "2cu4du CQI=${CQI} ${NUE}UE: "
    python3 - << PYEOF
import json, glob, numpy as np
files = glob.glob("${LOGDIR}/power_2cu4du_cqi${CQI}_${NUE}ue_run*.json")
means = []
