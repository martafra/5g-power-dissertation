...
#
# port layout, for UE index i (1-indexed), relative to --du-base-port (B):
#   DU   tx_port=B           rx_port=B+1
#   UEi  tx_port=B+(i*100)+1 rx_port=B+(i*100)
# e.g. with B=2000 (default): UE1=2101/2100, UE2=2201/2200, ... UE16=3601/3600
#
# B defaults to 2000 to stay backward compatible with the existing DU1
# automation (run_zmq_matrix_experiments.sh calls this with no --du-base-port
# flag). Use a different B (e.g. 10000) to run a second, fully independent
# broker instance for a second DU on the same host without any port clash
# with the first one.
...
class multi_ue_scenario_nue(gr.top_block):
    ...
    def __init__(self, n_ue=16, path_loss_db=None,
        ...
        self.du_base_port = du_base_port

        ##################################################
        # du source/sink (port offset depends on du_base_port, everything
        # else here is independent of n_ue)
        ##################################################
        du_tx_port = du_base_port
        du_rx_port = du_base_port + 1

        self.zeromq_req_source_du = zeromq.req_source(
            gr.sizeof_gr_complex, 1, f'tcp://127.0.0.1:{du_tx_port}',
            self.zmq_timeout, False, self.zmq_hwm)
        self.zeromq_rep_sink_du = zeromq.rep_sink(
            gr.sizeof_gr_complex, 1, f'tcp://127.0.0.1:{du_rx_port}',
            self.zmq_timeout, False, self.zmq_hwm)
        ...
        for i in range(1, n_ue + 1):
            tx_port = du_base_port + i * 100 + 1  # UE uplink source port (UE tx -> broker)
            rx_port = du_base_port + i * 100      # UE downlink sink port (broker -> UE rx)
...
def parse_args():
    ...
    parser.add_argument('--du-base-port', type=int, default=2000,
                         help='base port for the DU side and the UE port '
                              'formula offset (default: 2000, matches the '
                              'original single-broker DU1 setup). use a '
                              'different base, e.g. 10000, to run a second '
                              'independent broker instance for another DU '
                              'on the same host without clashing ports')
...
def main():
    ...
    tb = multi_ue_scenario_nue(
        ...
        du_base_port=args.du_base_port)
